/**
* 웹사이트 제작시 가장 널리 쓰이는 이미지, 클래스를 클릭 및 오버 하였을 경우
...
(function($){
    ...
    $.fn.yakuIcbtn = function(els, opts) {
        ...
        var elements = [];
        var selectedIndex = -1;
        var o = $.extend({},$.fn.yakuIcbtn.defaults,opts);

        return $(els).each(function(i) {
            var source = this.src;
            var $this  = $(this);
            var oLayer  = (o.layers[i]) ? o.layers[i] : null;
            var overSource = null;

            if(o.overType == "class" && o.clsName )  source = getClassName(o.clsName);
